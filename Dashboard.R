...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      menuItem("Customer Satisfaction Analysis", 
               ...
               icon = icon("star")),
      menuItem("Teammate Analysis 1", 
               tabName = "analysis1", 
               icon = icon("chart-line")),
      menuItem("Teammate Analysis 2", 
               tabName = "analysis2", 
               icon = icon("chart-bar")),
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      # Teammate Analysis 1 Tab
      tabItem(tabName = "analysis1",
              fluidRow(
                box(
                  title = "Teammate Analysis 1", 
                  status = "primary", 
                  solidHeader = TRUE,
                  width = 12,
                  h3("Analysis Title: [To be filled by teammate]"),
                  p("This section will contain the first teammate's analysis."),
                  br(),
                  div(
                    style = "text-align: center; padding: 50px;",
                    h4("Content will be added by Teammate 1", style = "color: #999;"),
                    icon("chart-line", style = "font-size: 48px; color: #ddd;")
                  )
                )
              ),
              fluidRow(
                column(6,
                       box(
                         ...
                         width = 12,
                         div(
                           style = "height: 300px; background-color: #f9f9f9; border: 2px dashed #ddd; 
                         display: flex; align-items: center; justify-content: center;",
                           p("Chart/Plot will go here", style = "color: #999; font-size: 18px;")
                         )
                       )
                ),
                column(6,
      ...
      # Teammate Analysis 2 Tab
      tabItem(tabName = "analysis2",
              fluidRow(
                box(
                  title = "Teammate Analysis 2", 
                  status = "primary", 
                  solidHeader = TRUE,
                  width = 12,
                  h3("Analysis Title: [To be filled by teammate]"),
                  p("This section will contain the second teammate's analysis."),
                  br(),
                  div(
                    style = "text-align: center; padding: 50px;",
                    h4("Content will be added by Teammate 2", style = "color: #999;"),
                ...
                column(6,
                       box(
                         ...
                         div(
                           style = "height: 300px; background-color: #f9f9f9; border: 2px dashed #ddd; 
                         display: flex; align-items: center; justify-content: center;",
                           p("Results will go here", style = "color: #999; font-size: 18px;")
                         )
                       )
                ),
                column(6,
                       box(
                         ...
                         div(
                           style = "height: 300px; background-color: #f9f9f9; border: 2px dashed #ddd; 
                         display: flex; align-items: center; justify-content: center;",
                           p("Summary will go here", style = "color: #999; font-size: 18px;")
                         )
                       )
                )
              )
...
server <- function(input, output, session) {
  ...
  generate_sample_data <- function() {
    set.seed(123)
    countries <- c("USA", "Canada", "UK", "Germany", "France", "Australia", "Japan")
    
    # Create sample data with different satisfaction rates per country
    data_list <- list()
    base_rates <- c(0.85, 0.78, 0.72, 0.68, 0.65, 0.82, 0.75)  # Different satisfaction rates
    
    for(i in 1:length(countries)) {
      n_samples <- sample(300:800, 1)  # Random sample size per country
      high_prob <- base_rates[i]
      
      ratings <- sample(c("High", "Low"), n_samples, 
                        replace = TRUE, 
                        prob = c(high_prob, 1-high_prob))
      
      data_list[[i]] <- data.frame(
        Country = countries[i],
        Ratings = ratings,
        Ratings_numeric = ifelse(ratings == "High", 5, 1)
      )
    }
  ...
  output$rating_distribution_plot <- renderPlotly({
    p <- ggplot(data, aes(x = Country, fill = Ratings)) +
      ...
      labs(title = "Customer Rating Distribution by Country", 
  ...
  output$chisq_results <- renderText({
    ...
    paste(
      ...
      "Chi-squared statistic:", round(chisq_result$statistic, 4), "\n",
  ...
  output$anova_results <- renderText({
    anova_result <- aov(Ratings_numeric ~ Country, data = data)
  ...
  output$model_performance_table <- DT::renderDataTable({
    ...
    performance_metrics <- data.frame(
      ...
      Value = c(0.785, 0.821, 0.743, 0.798),
      Description = c(
  ...
  output$prediction_plot <- renderPlotly({
    # Simulate prediction probabilities by country
    country_pred <- data %>%
      group_by(Country) %>%
      summarise(
        High_Rating_Probability = mean(Ratings == "High"),
        Sample_Size = n()
      )
  ...
  output$balanced_data_plot <- renderPlotly({
    ...
    sample_sizes <- data %>%
      ...
      mutate(Balanced_Count = 300)  # Simulated balanced count
  ...
  output$action_summary_table <- DT::renderDataTable({
    action_summary <- data.frame(
      Performance_Level = c("Good", "Average", "Needs Improvement"),
      Countries_Count = c(3, 2, 2),
      Action_Required = c("Maintain current practices", "Targeted improvements", "Immediate intervention")
    )
